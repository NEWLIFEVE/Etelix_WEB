
    <link href="slider2/plugins/bxslider/jquery.bxslider.css" rel="stylesheet" />                
    <link href="slider2/css/style.css" rel="stylesheet" type="text/css"/>

    <script type="text/javascript" src="slider2/plugins/bxslider/jquery.bxslider.min.js"></script>
    <script src="slider2/scripts/app.js"></script>


            <!-- BEGIN CLIENTS -->
            <div class="row margin-bottom-40 our-clients" style="margin:0 auto;">
                <div class="col-md-3">
                    <div class="principales_clientes">
                       <h4>Principales <br>Clientes</h4>    
                    </div>
                    
                </div>
                <div class="col-md-9">
                    <ul class="bxslider1 clients-list">
                        <li>
                            <a href="#">
                                <img alt="" src="<?php echo Yii::app()->request->baseUrl; ?>/images/1-01.png">
                                <img src="slider2/img/clients/client_1.png" class="color-img" alt="" />
                            </a>
                        </li>
                        <li>
                            <a href="#">
                                 <img alt="" src="<?php echo Yii::app()->request->baseUrl; ?>/images/2-01.png">
                                <img src="slider2/img/clients/client_2.png" class="color-img" alt="" />
                            </a>
                        </li>
                        <li>
                            <a href="#">
                                 <img alt="" src="<?php echo Yii::app()->request->baseUrl; ?>/images/3-01.png">
                                <img src="slider2/img/clients/client_3.png" class="color-img" alt="" />
                            </a>
                        </li>
                        <li>
                            <a href="#">
                                 <img alt="" src="<?php echo Yii::app()->request->baseUrl; ?>/images/4-01.png">
                                <img src="slider2/img/clients/client_4.png" class="color-img" alt="" />
                            </a>
                        </li>
                        <li>
                            <a href="#">
                                <img alt="" src="<?php echo Yii::app()->request->baseUrl; ?>/images/5-01.png">
                                <img src="slider2/img/clients/client_5.png" class="color-img" alt="" />
                            </a>
                        </li>
                        <li>
                            <a href="#">                        
                                 <img alt="" src="<?php echo Yii::app()->request->baseUrl; ?>/images/6-01.png">
                                <img src="slider2/img/clients/client_6.png" class="color-img" alt="" />
                            </a>
                        </li>
                        <li>
                            <a href="#">
                                 <img alt="" src="<?php echo Yii::app()->request->baseUrl; ?>/images/7-01.png">
                                <img src="slider2/img/clients/client_7.png" class="color-img" alt="" />
                            </a>
                        </li>
                        <li>
                            <a href="#">                        
                                 <img alt="" src="<?php echo Yii::app()->request->baseUrl; ?>/images/8-01.png">
                                <img src="slider2/img/clients/client_8.png" class="color-img" alt="" />
                            </a>
                        </li>
                         <li>
                            <a href="#">                        
                                 <img alt="" src="<?php echo Yii::app()->request->baseUrl; ?>/images/9-01.png">
                                <img src="slider2/img/clients/client_8.png" class="color-img" alt="" />
                            </a>
                        </li>
                         <li>
                            <a href="#">                        
                                 <img alt="" src="<?php echo Yii::app()->request->baseUrl; ?>/images/10-01.png">
                                <img src="slider2/img/clients/client_8.png" class="color-img" alt="" />
                            </a>
                        </li>
                         <li>
                            <a href="#">                        
                                 <img alt="" src="<?php echo Yii::app()->request->baseUrl; ?>/images/11-01.png">
                                <img src="slider2/img/clients/client_8.png" class="color-img" alt="" />
                            </a>
                        </li>
                         <li>
                            <a href="#">                        
                                 <img alt="" src="<?php echo Yii::app()->request->baseUrl; ?>/images/12-01.png">
                                <img src="slider2/img/clients/client_8.png" class="color-img" alt="" />
                            </a>
                        </li>
                         <li>
                            <a href="#">                        
                                 <img alt="" src="<?php echo Yii::app()->request->baseUrl; ?>/images/13-01.png">
                                <img src="slider2/img/clients/client_8.png" class="color-img" alt="" />
                            </a>
                        </li>
                         <li>
                            <a href="#">                        
                                 <img alt="" src="<?php echo Yii::app()->request->baseUrl; ?>/images/14-01.png">
                                <img src="slider2/img/clients/client_8.png" class="color-img" alt="" />
                            </a>
                        </li>
                    </ul>                        
                </div>
            </div>
            <!-- END CLIENTS -->

    <script type="text/javascript">
        jQuery(document).ready(function() {
            App.init();    
            App.initBxSlider();
            Index.initRevolutionSlider();                    
        });
    </script>
